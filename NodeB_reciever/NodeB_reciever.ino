...
// Replace with Node A’s MAC address
uint8_t nodeAMac[] = {0xF4, 0x65, 0x0B, 0xE8, 0x46, 0x5C};
uint8_t currentChannel = 1;

// ---------- Utility ----------
void printBlocks() {
  Serial.println("\n--- Node B Blocks (Current in RAM) ---");
  int startIdx = blockchain.size() > 10 ? blockchain.size() - 10 : 0;
  for (int i = startIdx; i < blockchain.size(); i++) {
    Block &b = blockchain[i];
    Serial.printf("#%d | %.2f cm | %lu ms\n", b.index, b.distance, b.timestamp);
  }
  Serial.printf("(Showing %d most recent blocks)\n", (int)(blockchain.size() - startIdx));
}

// ---------- ESP-NOW ----------
void sendAck() {
  String msg = String(SYNC_ACK) + "|ACK";
  esp_now_send(nodeAMac, (uint8_t *)msg.c_str(), msg.length() + 1);
}
...
void onReceive(const esp_now_recv_info_t *info, const uint8_t *data, int len) {
  ...
  if (type == NEW_BLOCK) {
    ...
    int b = payload.indexOf(',', a + 1);
    int c = payload.indexOf(',', b + 1);
    int d = payload.indexOf(',', c + 1);
    ...
    if (!syncComplete) {
      syncComplete = true;
      Serial.println("✅ Sync complete – receiving blocks");
    }
...
void setup() {
  ...
  delay(1000);  // let radio warm up before ESP-NOW
...
// ---------- Loop ----------
void loop() {
  // Everything handled in callbacks
}
